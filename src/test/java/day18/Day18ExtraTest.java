...
public class Day18ExtraTest implements AocCommon {
...
//    }
    ...
    public void testGridReduce() {
        ...
        var LIMIT  = 2914;
        for (var limit = LIMIT; limit < Math.min(LIMIT + 500, inputs.size()); limit++) {
            println("limit: " + limit);
            println("input: " + inputs.get(limit));
            
            var maze = new String[HEIGHT][];
            ...
            .map(grab(regex("[0-9]+")))
            .forEach(pair -> { 
                var col = parseInt(pair.get(0));
                var row = parseInt(pair.get(1));
                maze[row][col] = "#";
            });
...
//            
            ...
            if (pathLength < 0 || Math.abs(pathLength) > 23372036854775716L)
                break;
        }
    }
    ...
    void fillDeadEnds(Grid grid) {
        ...
        while (nodes.size() != 0) {
            ...
            if (((node.row == 0)         && (node.col == 0))
             || ((node.row == WIDTH - 1) && (node.col == HEIGHT - 1))) {
                continue;
            }
